...
def get_maps_by_lat_lng_buffer(lat, lng, zoom, radius):
    ...
    satellite_map, building_map = generate_map1(lat, lng, get_zoom_level_based_on_radius(radius), polygon)
...
def get_maps_by_polygon(city_name, radius, zoom):
    ...
    satellite_map, building_map = generate_map1(lat, lng, get_zoom_level_based_on_radius(radius), polygon, tooltip=name)
    area = dict(building_area=building_area, road_area=road_area, total_area=total_area)
    area_km2 = {}
    area_km2['name'] = name
    for key, value in area.items():
...
def _get_maps_by_polygon(city_name, radius, zoom):
    ...
    try:
        ...
        total_area_, building_area_, road_area_ = 0, 0, 0
...
def get_zoom_level_based_on_radius(radius):
...
def add_map_to_layout(map1, map2, lat, lng, zoom_level, area_info, marker=None):
    m = DualMap(location=(lat, lng), layout='horizontal', zoom_start=12, tiles=None)
    m.m1 = map1
    m.m2 = map2
    legend = get_legend()
...
if __name__ == "__main__":
    ...
    st.set_page_config(
        page_title="Spatic | Locality Overview",
        page_icon=favicon,
        layout="wide",
